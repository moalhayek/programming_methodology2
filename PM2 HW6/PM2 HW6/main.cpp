...
float shortestPathAnySrc(vector<vector<int> > graph, int weight[], int src);
...
int main(){
    ...
    float h = shortestPathAnySrc(arr, dist2, 0);
    ...
    cout << "\nImplementing shortest path algorithm for figure 20-39 with 2 as the origin." << endl;
        ...
        for (int u = 0; u<n; u++){
            if (!vertexSet[u]){
                if(weight[u] > weight[smallest] + graph[smallest][u]){
                    weight[u] = weight[smallest] + graph[smallest][u];
                    hop++;
                    cout << "Took a hop when going from " << src << " to " << smallest << " to " << u << endl;
                }
            }
        }
    }
